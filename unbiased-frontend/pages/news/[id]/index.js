...
export default function ShowNews() {
  ...
  useEffect(() => {
    if (comment) {
      const section = document.querySelector( `#comment-${comment}` );
      if (section) {
  ...
  return(
    <>
      <Head>
        <title>unbiased {id} - {article ? article.title : ''}</title>
            ...
            {loggedUser ? <div className="d-flex flex-row align-items-center gap-4px">
              ...
              <div className="ml-2 d-flex justify-content-center align-items-center">
                {   loggedUser && loggedUser.id === article.creator.id
                    ? <DeleteButton triggerEffect={articleTriggerEffect} creatorId={article.creator?.id} id={id} ></DeleteButton> : <></>}
              </div>
              <div className="ml-2 d-flex justify-content-center align-items-center">
                {loggedUser && loggedUser.id === article.creator.id ? <PinButton triggerEffect={articleTriggerEffect} creatorId={article.creator?.id} id={id} pinned={article.pinned}></PinButton> : <></>}
              </div>
            </div>: <></>}
          </div>
